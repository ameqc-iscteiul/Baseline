...
def eval_mujoco(ind:QDIndividual, evaluator : Optional[Evaluator], level : Optional[int]):
...
def evolution (args : Options()):
    ...
    evaluator = Evaluator()
    evaluator.set_options(args.descriptor_names, 0, args.vision_w, args.vision_h, args.budget)
    ...
    for i in range (args.levels):
        ...
        args.budget = args.budget/args.levels
        ...
        df.to_csv(Path(args.run_folder).joinpath(f"final_grid_{i}.csv"), index=False)
