...
class Runner(LocalRunner):
    ...
    def run(self):
        ...
        target_area_side = self.target_size + .042
        ...
        while ((time := self.data.time) < Config.simulation_time ) and target_reached is False:
            ...
            if target_area_x_interval[0] <= round(current_position[0],2) <= target_area_x_interval[1] and  target_area_y_interval[0] <= round(current_position[1],2) <= target_area_y_interval[1]: 
